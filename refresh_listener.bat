...
) else if "%MSG%"=="%STOPKM_SECRET%" (
    powershell -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File "%~dp0restart_powertoys.ps1" -Mode "Stop" -PowerToysPath "%POWERTOYS_PATH%"
) else if "%MSG%"=="%RESTARTKM_SECRET%" (
    powershell -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File "%~dp0restart_powertoys.ps1" -Mode "Restart" -PowerToysPath "%POWERTOYS_PATH%"
)
goto loop
